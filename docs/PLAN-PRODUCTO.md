...
### Estado de P5 · 31 de agosto de 2026

| | Qué es | Estado |
|---|---|---|
| P5a | **Quién ha abandonado** | ✅ cuatro estados, y la lista ordenada por urgencia |
...
que en el panel del alumno porque con estos números se decide **a quién se llama
por teléfono**. Las tres decisiones que importan están en la regla 35 de
[`CLAUDE.md`](../CLAUDE.md).

Lo primero que enseñó al conectarlo a los datos reales: **43 de los 45 temas no
tienen ni una pregunta**. Eso no es un fallo del panel, es el panel haciendo su
trabajo.
...
| P11d | Rol `superadmin`, distinto de `admin` por academia | ✅ **cerrada** (12 sep, afinada tras probarla en vivo): `AuthUser.role` admite `'superadmin'`, y `organizationId` es SIEMPRE `null` para él — nunca hereda una academia por ser miembro de ella (regla 65). Administra el banco COMÚN (temario + moderación) y ve la comparativa transversal; nada de una academia en concreto. `gaepmalaga@gmail.com` ya tiene el rol en producción; `morato@atenea.com` se queda `admin` |
...
| P11f | Panel de superadmin: alumnos, rentabilidad y moderación cruzados | ✅ **cerrada, afinada tras probarla en vivo** (12 sep): pestaña **«Academias»**, solo visible para `superadmin` (`AdminAcademies.tsx`). Alumnos y admins por academia, coste de IA (cruzando `ai_usage` contra `academy_members`, que no lleva `organization_id` a propósito) y lo cobrado este mes (`monthly_payments`, que sí lo lleva). La moderación cruzada ya la resuelve P11e — no se duplicó la pantalla. **El primer despliegue enseñaba de más**: el superadmin heredaba la academia de su `academy_members` (Alphapol, del backfill) y veía Alumnos/Grupos/Pagos/Prep. física como un admin cualquiera. Corregido: su `organizationId` es siempre `null`, y sus pestañas se reducen a Temario & IA + Moderación + Academias — el banco común, nada de una academia en concreto. De paso, `getAiCostOverview` (que no estaba acotado) pasó a filtrar por academia para un `admin` normal, igual que el resto del panel (regla 65) |
