...
import { 
  ...
  Layers, CheckCircle2, XCircle // <--- ESTOS FALTABAN, AHORA ESTÁN AÑADIDOS
} from 'lucide-react';

export default function AdminView({ user, onLogout }: { user: any, onLogout: () => void }) {
  // Tabs: 'users', 'activity', 'topics', 'debug'
  ...
  const [dbHealth, setDbHealth] = useState<any>(null); // Estado para el diagnóstico
...
async function loadTopics() {
  ...
  async function handleFileUpload(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    if (res.success) {
      alert("✅ " + res.message);
      await loadTopics(); // Recargar lista visual
    } else {
      alert("❌ " + res.error);
    }
  }
    ...
    <div className="min-h-screen bg-slate-900 text-slate-200 p-4 md:p-8 font-sans">
      ...
      <header className="flex flex-col md:flex-row justify-between items-center mb-8 pb-6 border-b border-slate-800 gap-4">
        ...
        <div className="flex items-center gap-3">
            ...
            <button onClick={onLogout} className="flex items-center gap-2 px-4 py-2 rounded-lg hover:bg-slate-800 text-slate-400 hover:text-white transition-colors border border-transparent hover:border-slate-700">
      ...
      <div className="flex flex-wrap gap-2 mb-6 bg-slate-800/50 p-1 rounded-xl w-fit border border-slate-700/50">
         ...
         {/* Botón Pestaña Diagnóstico (Rojo para destacar) */}
         <button 
               ...
               <tbody className="divide-y divide-slate-700">
                  {usersList.map((u:any) => (
                     <tr key={u.id} className="hover:bg-slate-700/50 transition-colors">
                        ...
                        <td className="px-6 py-4"><span className={`px-2 py-1 rounded text-[10px] font-black uppercase ${u.role==='admin'?'bg-yellow-500 text-black':'bg-slate-600 text-white'}`}>{u.role}</span></td>
                        <td className="px-6 py-4 text-center font-mono">{u.total_tests}</td>
                        <td className="px-6 py-4 text-center"><span className={`font-bold ${u.win_rate>=50?'text-green-400':'text-red-400'}`}>{u.win_rate}%</span></td>
                        <td className="px-6 py-4 text-green-400 flex items-center gap-2"><div className="w-1.5 h-1.5 bg-green-500 rounded-full"></div> Activo</td>
                     </tr>
                  ))}
               </tbody>
            </table>
         ...
         <div className="space-y-6 animate-in fade-in">
            ...
            <div className="bg-slate-800 p-8 rounded-2xl border border-slate-700 border-dashed text-center">
               {uploading ? (
                  <div className="flex flex-col items-center gap-3 animate-pulse">
                     ...
                     <p className="text-slate-400">Procesando PDF, leyendo artículos e indexando en IA...<br/><span className="text-xs opacity-70">(Esto puede tardar un poco si el archivo es grande)</span></p>
                  </div>
               ) : (
                  <>
            ...
            <div className="bg-slate-800 rounded-2xl border border-slate-700 overflow-hidden">
               <div className="px-6 py-4 bg-slate-900/50 border-b border-slate-700 font-bold text-sm text-slate-400 uppercase flex justify-between items-center">
                  ...
                  <button onClick={loadTopics} className="hover:text-white"><RefreshCw size={14}/></button>
               </div>
               <div className="divide-y divide-slate-700">
                  ...
                  {topicsList.length === 0 && (
                     <div className="p-8 text-center text-slate-500">
                        ...
                        <p className="text-xs text-slate-600">¿Has subido un archivo y no aparece aquí? Revisa la pestaña <span className="text-red-400 font-bold">Diagnóstico DB</span>.</p>
                     </div>
                  )}
               </div>
            </div>
        ...
        <div className="space-y-6 animate-in fade-in">
            <div className="bg-slate-800 p-6 rounded-2xl border border-red-900/50 shadow-2xl">
                <div className="flex items-center gap-3 mb-6">
                    ...
                    <button onClick={runDiagnosis} className="ml-auto bg-slate-700 hover:bg-slate-600 text-white px-4 py-2 rounded-lg font-bold text-xs flex items-center gap-2">
